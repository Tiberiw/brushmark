...
from interfaces import Configuration, LossFunction, Optimizer
...
def get_optimizer(cfg: Optimizer, model: nn.Module):
    name = cfg.name
...
def get_loss_fn(cfg: LossFunction, class_weights: dict[int, float], n_classes: int, device: torch.device) -> nn.Module:
